...
# the steps along the unix_time scale to generate the new lines
df_steps =
  with(df_parm, {
  tibble( unix_time  = min_ts -(step * steps_extra) + seq( 1:(n + steps_extra * 2))* step,
          row_n = (1-steps_extra): (n + steps_extra))
            })
...
# add in the 3 predicted lines, for all the steps
df_steps %>%
...
# pull out the size of the triangles
...
df_steps %>%
  filter( predicted_end > 0 & unix_time <= df_parm$max_ts  ) %>% 
